...
Future<List<dynamic>> fetchRooms(String token) async {
  final String? _VIDEOSDK_API_ENDPOINT = dotenv.env['VIDEOSDK_API_ENDPOINT'];
  ...
  List<dynamic> rooms = jsonDecode(meetingIdResponse.body)['data'];
  return rooms;
}

Future<List<dynamic>> fetchRecordings(String token, String meetingId) async {
  final String? _VIDEOSDK_API_ENDPOINT = dotenv.env['VIDEOSDK_API_ENDPOINT'];

  final Uri getMeetingIdUrl =
      Uri.parse('$_VIDEOSDK_API_ENDPOINT/recordings?roomId=$meetingId');
  final http.Response meetingIdResponse =
      await http.get(getMeetingIdUrl, headers: {
    "Authorization": token,
  });
  List<dynamic> recordes = jsonDecode(meetingIdResponse.body)['data'];
  return recordes;
}
